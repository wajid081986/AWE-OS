...
// Formula: balance_n = balance_{n-1} × (1 + r) + deposit
// Interest earned in year n = balance_{n-1} × r (on opening balance)
...
// Withdrawal: allowed from year 7 onwards (i.e., after 6 completed years).
// Max = 50% × min(balance at end of Year 4, balance at end of preceding year).
...
export default function PPFCalculator() {
  ...
  const SEO_TITLE = 'PPF Calculator — Public Provident Fund Calculator India | AWE-OS'
  const SEO_DESC  = 'Calculate PPF maturity amount, interest, 80C tax savings, partial withdrawals and 5-year extensions. Year-by-year breakdown with charts. Free online PPF calculator for India.'
  const SEO_URL   = 'https://www.awe-os.com/tools/ppf-calculator'
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="keywords"           content="ppf calculator, public provident fund calculator, ppf interest calculator, ppf maturity calculator, ppf 80c deduction, ppf withdrawal calculator, ppf extension calculator, india" />
        ...
        <script type="application/ld+json">{JSON.stringify({
          ...
          name: 'PPF Calculator', url: SEO_URL,
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900">
        ...
        <div className="border-b border-white/10 bg-black/20 backdrop-blur-sm">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-2 text-xs text-gray-400">
            ...
            <span>/</span>
            <span className="text-gray-300">PPF Calculator</span>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10 space-y-6">
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 rounded-full px-4 py-1.5 text-xs text-emerald-300 font-medium mb-4">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              Government-Backed · EEE Tax Status · Risk-Free
            </div>
            <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3 tracking-tight">PPF Calculator</h1>
            <p className="text-gray-400 text-sm sm:text-base max-w-xl mx-auto leading-relaxed">
              Plan your Public Provident Fund journey — maturity amount, year-by-year growth,
              80C tax savings, withdrawal eligibility &amp; extension projections.
            </p>
            ...
            {FAQS.map((faq, i) => (
              <div key={i} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                ...
                >
                  ...
                  <span className={`text-gray-400 flex-shrink-0 text-xs transition-transform duration-200 ${openFaq === i ? 'rotate-180' : ''}`}>▼</span>
                </button>
                {openFaq === i && (
