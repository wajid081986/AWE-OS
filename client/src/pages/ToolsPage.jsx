  ...
  if (comingSoon) {
    return (
      <div className="flex items-center gap-3 p-4 bg-gray-50 border border-gray-200 rounded-xl cursor-not-allowed select-none">
        <span className="text-2xl shrink-0 opacity-50">{icon}</span>
...
export default function ToolsPage() {
  ...
  return (
    <>
      ...
      {/* Page hero */}
      <div className="bg-gradient-to-b from-gray-50 to-white border-b border-gray-200 px-4 py-8">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* ── Category tabs ──────────────────────────────── */}
        <div className="flex gap-1.5 flex-wrap mb-8 pb-4 border-b border-gray-100">
          {TAB_KEYS.map(tab => {
          ...
          {activeTab === 'all' && (
            <div className="flex gap-2 ml-auto">
              <input
                ...
                className="px-3 py-1.5 border border-gray-200 rounded-full text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 w-44"
              />
              <select
                value={sort}
                onChange={e => setFilter('sort', e.target.value)}
                className="px-3 py-1.5 border border-gray-200 rounded-full text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
              >
